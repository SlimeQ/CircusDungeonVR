...
public class RedistCopy {
	...
	public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject) {
		...
		if (target != BuildTarget.StandaloneWindows && target != BuildTarget.StandaloneWindows64 &&
            //!@
			target != BuildTarget.StandaloneOSXIntel && target != BuildTarget.StandaloneOSXIntel64 && target != BuildTarget.StandaloneOSXIntel &&
		...
		if (File.Exists(controllerCfg)) {
            ...
            if (target == BuildTarget.StandaloneOSXIntel || target == BuildTarget.StandaloneOSXIntel64 || target == BuildTarget.StandaloneOSXIntel) {
				dir = ".app/Contents";
			}
